...
def chat():
    ...
    if not api_key:
        return {"error": "ANTHROPIC_API_KEY が設定されていません。.env ファイルを確認してください。"}, 500
    ...
    def generate():
        try:
            ...
            yield f"data: {json.dumps({'type': 'error', 'text': 'APIキーが無効です。ANTHROPIC_API_KEY を確認してください。'})}\n\n"
